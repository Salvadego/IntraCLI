...
type CalConfig struct {
	...
	Month      int
	Force      bool
	FilterName string // raw qlvm query or @savedName
...
var (
	colorRed    = hex2rgb("#ea6962")
	colorYellow = hex2rgb("#e78a4e")
	colorGreen  = hex2rgb("#a9b665")
)
...
func (r Renderer) colorForHours(journeyHours, dayHours float64) string {
	...
	var c RGB
	if t <= 0.5 {
		c = lerp(colorRed, colorYellow, t/0.5)
	} else {
		c = lerp(colorYellow, colorGreen, (t-0.5)/0.5)
	}
	return fmt.Sprintf("\033[38;2;%d;%d;%dm", c.R, c.G, c.B)
}

func (r Renderer) RenderMonth(year int, month time.Month, days []DayInfo, journeyHours float64) {
...
func (r Renderer) printHeader(year int, month time.Month) {
	...
	if len(title) < totalWidth {
		fmt.Printf("%s%s\n", strings.Repeat(" ", (totalWidth-len(title))/2), title)
	} else {
		fmt.Println(title)
	}
}
...
func (r Renderer) formatDay(d DayInfo, journeyHours float64) string {
	...
	if d.IsToday {
		return fmt.Sprintf("%s%s%s%s%s%s", bold, invert, color, cell, reset, strings.Repeat(" ", r.Padding))
	}
	return fmt.Sprintf("%s%s%s%s%s", bold, color, cell, reset, strings.Repeat(" ", r.Padding))
}

func (r Renderer) RenderDay(year int, month time.Month, day int, info DayInfo, nonBusiness map[int]mantis.NonBusinessDay) {
...
func init() {
	...
	calCmd.Flags().IntVar(&calCfg.Year, "year", n.Year(), "Year")
	calCmd.Flags().IntVar(&calCfg.Month, "month", int(n.Month()), "Month (1-12)")
	calCmd.Flags().BoolVarP(&calCfg.Force, "force", "f", false, "Force refresh")
	calCmd.Flags().StringVarP(&calCfg.FilterName, "filter", "F", "", "Filter: raw qlvm query or @savedName")
...
var calCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// Resolve the optional filter query string once up front.
		filterQuery := resolveFilter(calCfg.FilterName, appConfig.SavedFilters)
		...
		if calCfg.YearView {
			...
			for m := time.January; m <= time.December+1; m++ {
				realMonth, realYear := m, calCfg.Year
				if m > time.December {
					realMonth, realYear = time.January, calCfg.Year+1
				}
				...
				if !calCfg.Force {
					ts, _ = cache.ReadFromCache[mantis.TimesheetsResponse](tsFilename)
				}

				if calCfg.Force || len(ts) == 0 {
					ts, err = mantisClient.Timesheet.GetTimesheets(mantisCtx, currentUserID, realYear, realMonth)
					if err != nil {
				...
				ts = utils.ApplyFilter(ts, filterQuery, profile)
				...
				if !calCfg.Force {
					nbResp, _ = cache.ReadFromCache[mantis.NonBusinessDay](nbFilename)
				}
				if calCfg.Force || len(nbResp) == 0 {
					...
					if err != nil {
						...
						nbResp = fetched
						_ = cache.WriteToCache(nbFilename, nbResp)
					}
				}
			...
			for m := time.January; m <= time.December+1; m++ {
				...
				for _, ts := range timesheetsByMonth[srcYM] {
					...
					dstYM := YearMonth{Year: parsedDate.Year(), Month: parsedDate.Month()}
					if _, ok := hoursByDateByMonth[dstYM]; ok {
						hoursByDateByMonth[dstYM][key] += ts.Quantity
						dateAppointmentsByMonth[dstYM][key] = append(dateAppointmentsByMonth[dstYM][key], ts)
					}
				}
			}
			...
			for m := time.January; m <= time.December; m++ {
				...
				allDays[m] = buildDays(calCfg.Year, m,
		...
		getTimesheets := func(y int, m time.Month) []mantis.TimesheetsResponse {
			...
			if !calCfg.Force {
				ts, _ = cache.ReadFromCache[mantis.TimesheetsResponse](filename)
			}
			if calCfg.Force || len(ts) == 0 {
				ts, err = mantisClient.Timesheet.GetTimesheets(mantisCtx, currentUserID, y, m)
				if err == nil {
		...
		timesheets := append(getTimesheets(year, month), getTimesheets(nextYear, nextMonth)...)
		timesheets = utils.ApplyFilter(timesheets, filterQuery, profile)
		...
		if calCfg.Force || len(nonBusinessResp) == 0 {
			nonBusinessResp, _ = mantisClient.Calendar.GetNonBusinessDays(mantisCtx, year, month)
		...
		days := buildDays(year, month, hoursByDate, dateAppointments, nonBusiness, time.Now())
...
func (r Renderer) RenderYear(year int, allDays map[time.Month][]DayInfo, journeyHours float64) {
	monthsPerRow := 3
	cellWidth := 2 + r.Padding
	...
	for row := range 4 {
		...
		for m := start; m <= end; m++ {
			fmt.Printf("%-*s", monthWidth, center(mesesLong[m-1], monthWidth))
		}
		fmt.Println()
...
func timesheetCacheKey(userID, year int, month time.Month) string {
